...
Hard filters (maxPrice, maxDeadline) eliminate ineligible bids \emph{before} scoring. The LLM commentary (optional, via Anthropic API) can explain the winner but cannot change it.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        ...
        \caption{Vault status check showing PickedUp state.}
    \end{subfigure}
...
The frontend exposes two AI-assisted actions that call the server-side agent API:

\begin{itemize}
    \item \textbf{✦ AI Pick Best Bid:} calls \texttt{POST /api/agent/rank} with the loaded bids. The server runs \texttt{SellerAgent.rank()}, optionally calls the Claude API for a plain-English explanation, and returns the winner index. The bid index field auto-fills and the explanation appears in a purple card.
    \item \textbf{✦ AI Auto-Confirm (Mailbox Agent):} calls \texttt{POST /api/agent/mailbox-confirm}. The server checks sensor state, rate-limits, and submits the \texttt{confirmDelivery} transaction on the user's behalf.
\end{itemize}
