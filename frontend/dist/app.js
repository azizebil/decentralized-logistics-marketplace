...
const FLOW_STEPS = ["Request", "Bids", "Agent", "Fund", "Pickup", "Mailbox", "Final"];
const AGENT_ALPHA = 1.5;
...
const REGISTRY_ABI = [
  ...
  "function agentPolicies(address) view returns (address agent,uint256 maxPrice,uint256 minDeadlineBuffer,uint256 coSignThreshold,bool enabled)",
...
function updateAddresses() {
  ...
  $("agentAccountAddress").textContent = wallet(Number($("agentAccount").value)).address;
  $("operatorAddress").textContent = wallet(Number($("operatorAccount").value)).address;
  ...
  if (!$("agentAddress").value) $("agentAddress").value = $("agentAccountAddress").textContent;
}
...
function setVaultAddress(address) {
  if (address && address !== ethers.ZeroAddress) {
    ...
    localStorage.removeItem("dlm.vaultAddress");
  }
}
...
async function refreshDetails() {
  ...
  const pol = await readRegistry.agentPolicies(r.seller);
  state.selectedRequest = r;
  ...
  const out = {
    ...
    agentPolicy: {
      enabled: pol.enabled,
      agent: pol.agent,
      maxPrice: `${ethers.formatEther(pol.maxPrice)} ETH`,
      minDeadlineBuffer: `${pol.minDeadlineBuffer}s`,
      coSignThreshold: `${ethers.formatEther(pol.coSignThreshold)} ETH`
    }
  };
  renderFlow(out.stage, state.selectedVault?.state);
  $("detailsOutput").textContent = format(out);
  if (r.vault !== ethers.ZeroAddress) await refreshVaultStatus(false);
}
...
async function refreshVaultStatus(writeLog = true) {
  ...
  state.selectedVault = out;
  renderFlow(STAGES[Number(state.selectedRequest?.stage ?? 0)], out.state);
...
function renderFlow(registryStage = "None", vaultState = null) {
  const stepIds = ["stepRequest", "stepBid", "stepAgent", "stepFund", "stepPickup", "stepMailbox", "stepFinal"];
  let active = 0;
  let failed = false;

  if (registryStage === "Open") active = 1;
  if (registryStage === "Assigned" || registryStage === "Held") active = 2;
  if (registryStage === "Finalized") active = 6;
  if (registryStage === "Cancelled") {
    active = 6;
    failed = true;
  }

  if (vaultState === "Funded") active = 3;
  if (vaultState === "PickedUp") active = 4;
  if (vaultState === "Delivered") active = 5;
  if (vaultState === "Refunded" || vaultState === "Failed") {
    active = 6;
  ...
  stepIds.forEach((id, index) => {
    const el = $(id);
    el.classList.toggle("complete", index < active && !failed);
    el.classList.toggle("active", index === active && !failed);
    el.classList.toggle("failed", index === active && failed);
    el.setAttribute("aria-label", `${FLOW_STEPS[index]} ${index < active ? "complete" : index === active ? "active" : "pending"}`);
  });
}
...
async function loadBids() {
  const reg = registry("sellerAccount");
  const id = selectedId();
  const [request, bids] = await Promise.all([
    reg.getRequest(id),
    reg.getBids(id)
  ]);
  state.selectedRequest = request;
  const decision = rankBids(bids, request);
  state.lastDecision = decision;
  renderDecision(decision, request);
  $("bidsList").innerHTML = "";
  bids.forEach((bid, index) => {
    const el = document.createElement("div");
    const scored = decision.scored.find((s) => s.index === index);
    el.className = `item${decision.winnerIndex === index ? " winner" : ""}${bid.withdrawn ? " withdrawn" : ""}`;
    el.innerHTML = `<strong>Bid ${index}${bid.withdrawn ? " - withdrawn" : ""}</strong>
      Courier: ${bid.courier}<br>
      Payout: ${bid.payout}<br>
      Price: ${ethers.formatEther(bid.price)} ETH<br>
      Promised: ${dateOf(bid.promisedTime)}<br>
      Reputation: ${Number(bid.reputationE4) / 100}%
      <div class="score-row">
        <span class="pill ${decision.winnerIndex === index ? "good" : ""}">score ${formatScore(scored?.score)}</span>
        <span class="pill ${scored?.eligible ? "good" : "warn"}">${scored?.reason || "not scored"}</span>
      </div>`;
    $("bidsList").appendChild(el);
  });
  if (decision.winnerIndex >= 0) $("acceptBidIndex").value = String(decision.winnerIndex);
  log(`Loaded and scored ${bids.length} bids`, {
    preferTrusted: request.preferTrusted,
    winnerIndex: decision.winnerIndex,
    formula: request.preferTrusted ? `r^${AGENT_ALPHA}/(t*p)` : "1/(t*p)"
  });
}

async function rankAndFill() {
  await loadBids();
  if (state.lastDecision?.winnerIndex >= 0) {
    $("acceptBidIndex").value = String(state.lastDecision.winnerIndex);
    log("Agent rank filled bid index", state.lastDecision);
  }
}

function rankBids(bids, request) {
  const nowSec = Math.floor(Date.now() / 1000);
  const preferTrusted = request.preferTrusted === true;
  const scored = bids.map((bid, index) => {
    if (bid.withdrawn) {
      return { index, score: -Infinity, eligible: false, reason: "withdrawn" };
    }
    if (bid.price > request.maxPrice) {
      return { index, score: -Infinity, eligible: false, reason: "price > budget" };
    }
    if (bid.promisedTime > request.maxDeadline) {
      return { index, score: -Infinity, eligible: false, reason: "promise > deadline" };
    }
    const secondsRemaining = Number(bid.promisedTime) - nowSec;
    if (secondsRemaining <= 0) {
      return { index, score: -Infinity, eligible: false, reason: "promise expired" };
    }
    const reputation = Number(bid.reputationE4) / 10000;
    const repFactor = preferTrusted ? Math.pow(Math.max(reputation, 0.000001), AGENT_ALPHA) : 1;
    const priceWei = Number(bid.price);
    const score = repFactor / (secondsRemaining * priceWei);
    return {
      index,
      score,
      eligible: true,
      secondsRemaining,
      repFactor,
      reason: preferTrusted ? `r^${AGENT_ALPHA}/(t*p)` : "1/(t*p)"
    };
  });

  const winner = scored.reduce((best, item) => item.score > best.score ? item : best, { index: -1, score: -Infinity });
  return {
    winnerIndex: winner.score === -Infinity ? -1 : winner.index,
    scored
  };
}

function renderDecision(decision, request) {
  const formula = request.preferTrusted ? `r^${AGENT_ALPHA}/(t*p)` : "1/(t*p)";
  const winner = decision.scored.find((item) => item.index === decision.winnerIndex);
  $("agentDecisionOutput").innerHTML = decision.winnerIndex >= 0
    ? `<strong>Seller Agent Decision</strong>
       Winner bid ${decision.winnerIndex} / formula ${formula} / score ${formatScore(winner.score)}`
    : `<strong>Seller Agent Decision</strong>No eligible bid under seller limits.`;
}

function formatScore(score) {
  if (score === undefined || score === -Infinity) return "ineligible";
  return (score * 1e18).toExponential(3);
}
...
async function acceptBidAsAgent() {
  const reg = registry("agentAccount");
  const receipt = await waitTx(await reg.acceptBidByAgent(selectedId(), Number($("acceptBidIndex").value)), "Accept bid as agent");
  const parsed = receipt.logs.map((l) => { try { return reg.interface.parseLog(l); } catch { return null; } })
    .find((event) => event?.name === "BidAccepted");
  if (parsed) setVaultAddress(parsed.args.vault);
}

function selectAgentKey() {
  $("agentAddress").value = signer("agentAccount").address;
  log("Selected seller session key", { agent: $("agentAddress").value });
}

async function setAgent() {
  if (!$("agentAddress").value.trim()) selectAgentKey();
...
function bind(id, fn) {
  $(id).addEventListener("click", () => Promise.resolve(fn()).catch(fail));
}
...
async function init() {
  ...
  persistInputs();
  for (const id of ["sellerAccount", "courierAccount", "buyerAccount", "mailboxAccount", "agentAccount", "operatorAccount"]) {
  ...
  bind("rankBidsBtn", rankAndFill);
  bind("acceptBidBtn", acceptBid);
  bind("acceptBidAgentBtn", acceptBidAsAgent);
  bind("useAgentBtn", selectAgentKey);
